...
import { login as beLoginApi, saveAccessToken, clearAccessToken, me as beMe } from "@/integrations/api/auth";
...
interface AuthContextType {
  ...
  beLogin: (email: string, password: string) => Promise<boolean>;
  beLogout: () => void;
  beToken: string | null;
}
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const [beToken, setBeToken] = useState<string | null>(() => {
  ...
  // Récupérer le rôle depuis le backend quand un JWT est présent
  useEffect(() => {
    let cancelled = false;
    (async () => {
      if (!beToken) return;
      try {
        ...
        if (!cancelled && meResp && typeof meResp.role === 'string') {
          setDbRole(meResp.role);
        }
      } catch (err) {
        // non bloquant
      } finally {
  ...
  const beLogin = async (email: string, password: string) => {
    try {
      const resp = await beLoginApi(email, password);
      const token = (resp as any)?.access_token as string | undefined;
      if (typeof token === "string" && token.length > 0) {
        // Ensure token is persisted and state updated
        try { saveAccessToken(token); } catch { /* empty */ }
        setBeToken(token);
        return true;
      }
      return false;
  ...
  const beLogout = () => {
    try { clearAccessToken(); } catch { /* empty */ }
    setBeToken(null);
  };
  ...
  const signIn = async (email: string, password: string) => {
    ...
    // Si login Supabase réussi, connecter aussi le backend pour obtenir le JWT
    try {
      if (!result.error) {
        const resp = await beLoginApi(email, password);
        const token = (resp as any)?.access_token as string | undefined;
      ...
      // Non bloquant: l'app peut continuer mais les appels backend protégés échoueront sans JWT
      console.warn('Backend login failed after Supabase login:', e);
    }
    return result;
  };
