...
import { MapPin, Calendar, Eye, ShieldCheck, ShieldAlert } from "lucide-react";
...
export default function CandidateApplications() {
  ...
  const { verifyMatricule } = useBackendAuth();
  const [matriculeStatus, setMatriculeStatus] = useState<"unknown" | "valid" | "invalid" | "error">("unknown");
  ...
  const onVerifyMatricule = async () => {
    try {
      setChecking(true);
      const res = await verifyMatricule();
      if (res.valid) {
        setMatriculeStatus("valid");
        toast.success("Matricule vérifié");
      } else {
        setMatriculeStatus("invalid");
        toast.warning(res.reason || "Matricule non valide");
      }
    } catch {
      setMatriculeStatus("error");
      toast.error("Erreur lors de la vérification du matricule");
    } finally {
      setChecking(false);
    }
  };
      ...
      <div className="space-y-6 sm:space-y-8 px-3 sm:px-0">
        <div className="flex items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            <Button variant="outline" size="sm" onClick={onVerifyMatricule} disabled={checking}>
              {checking ? (
                <>
                  <ShieldCheck className="w-4 h-4 mr-2 animate-pulse" /> Vérification...
                </>
              ) : (
                <>
                  <ShieldCheck className="w-4 h-4 mr-2" /> Vérifier mon matricule
                </>
              )}
            </Button>
            {matriculeStatus === "valid" && (
              <Badge className="bg-green-600">Valide</Badge>
            )}
            {matriculeStatus === "invalid" && (
              <Badge variant="destructive" className="gap-1">
                <ShieldAlert className="w-3 h-3" /> Invalide
              </Badge>
            )}
          </div>
        </div>
