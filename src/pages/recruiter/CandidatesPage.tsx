    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <div className="mt-2 grid grid-cols-1 sm:grid-cols-2 gap-2">
          {(documents || []).map((doc) => (
            <a key={String(doc.id)} href={toUrl(doc.file_url)} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 text-primary hover:underline">
              <FileText className="w-4 h-4" /> {getDocumentTypeLabel(doc.document_type || '')}: {doc.file_name}
            </a>
          ))}
        </div>
      </div>
...
export default function CandidatesPage() {
  ...
  useEffect(() => {
    // Debug supprimé: utilisation de l'API backend désormais
  }, []);
