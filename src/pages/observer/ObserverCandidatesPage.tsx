      ...
      <div className="space-y-3">
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {documents.map((doc) => (
              <div key={String(doc.id)} className="flex items-center justify-between p-3 border rounded-lg bg-card">
                ...
                <div className="flex gap-2">
                  <Button variant="outline" size="sm" onClick={() => window.open(toUrl(doc.file_url), '_blank')}>
...
export default function ObserverCandidatesPage() {
  ...
  const candidates: UICandidate[] = useMemo(() => {
    ...
    return applications.map(app => ({
      ...
      status: (app.status as ApplicationStatus) || 'candidature',
