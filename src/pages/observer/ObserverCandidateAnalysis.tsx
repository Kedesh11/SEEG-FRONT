  ...
  const handleDownloadAll = async () => {
    try {
      if (documents && documents.length > 0) {
        ...
        toast({ title: "Téléchargement réussi", description: "Tous les documents ont été téléchargés en ZIP" });
      }
    } catch (error) {
      toast({ title: "Erreur de téléchargement", description: "Impossible de télécharger les documents", variant: "destructive" });
    }
  };
      ...
      <CardContent className="p-4 sm:p-6">
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {documents.map((doc) => (
              <div key={String(doc.id)} className="border rounded-lg p-4 bg-card">
                ...
                <div className="flex gap-2">
                  <Button variant="outline" size="sm" className="flex-1" onClick={() => window.open(toUrl(doc.file_url), '_blank')}>
...
const ProtocolsTab = ({ application }: { application: Application }) => {
  ...
  useEffect(() => {
    // Intégration backend à venir: utiliser les endpoints evaluations
    setIsLoading(false);
  }, [application.id]);
